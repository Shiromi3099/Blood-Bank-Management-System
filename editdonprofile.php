...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    // Retrieve updated profile data from the form and sanitize inputs
    $fullname = mysqli_real_escape_string($conn, $_POST['fullname']);
    $age = mysqli_real_escape_string($conn, $_POST['age']);
    $email = mysqli_real_escape_string($conn, $_POST['email']);
    $mob = mysqli_real_escape_string($conn, $_POST['mob']);
    $btype = mysqli_real_escape_string($conn, $_POST['btype']);
    $mhistory = mysqli_real_escape_string($conn, $_POST['mhistory']);

    // SQL query to update user information based on UID
    $update_sql = "UPDATE users u 
                    INNER JOIN donor d ON u.uid = d.uid 
                    SET u.name = '$fullname', u.email = '$email', u.mobno = '$mob', 
                        d.age = '$age', d.bloodtype = '$btype', d.mhistory = '$mhistory' 
                    WHERE u.uid = $uid";

    if ($conn->query($update_sql) === TRUE) {
        // Redirect to the profile page after successful update
        header("Location: donprofile.php");
        exit();
    } else {
...
// SQL query to fetch user information based on UID
$sql = "SELECT u.name, u.email, u.mobno, d.age, d.bloodtype, d.mhistory
        FROM users u 
        INNER JOIN donor d ON u.uid = d.uid 
        WHERE u.uid = $uid";
                                        ...
                                        <div class="card-body">
                                            <div class="d-flex flex-column align-items-center text-center">
                                                ...
                                                <div class="mt-3">
                                                    ...
                                                    <p class="text-secondary mb-1">Blood Donor</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="card-body">
                                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                                        ...
                                        <div class="row mb-3">
                                            ...
                                            <div class="col-sm-9 text-secondary">
                                                <input type="text" class="form-control" name="mhistory" value="<?php echo $mhistory; ?>">
                                            </div>
                                        </div>
                                        <div class="row">
